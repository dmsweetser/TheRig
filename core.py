...
if not os.path.exists(get_config("log_folder",False)):
    os.makedirs(get_config("log_folder",False))

logger = CustomLogger(get_config("log_folder",False))

# Set defaults if not present in the config
app.secret_key = get_config('secret_key',False)
app.config['MODEL_FOLDER'] = get_config('model_folder',False)
app.config['MODEL_URL'] = get_config('model_url',False)
app.config['MAX_CONTEXT'] = get_config('n_ctx',False)
app.config['REVISIONS_PER_PAGE'] = get_config('revisions_per_page',False)
app.config['SESSION_TYPE'] = get_config('session_type',False)
app.config['MAX_FILE_SIZE'] = get_config('max_file_size',False)

@app.route('/process_request_creative', methods=['POST'])
def process_request_creative():
    return process_request(True)

@app.route('/process_request_cleanup', methods=['POST'])
def process_request_cleanup():
    return process_request(False)


def process_request(is_creative):
    ...
    # Save the dictionary as a JSON file with the current ticks as the name
    with open(f"{get_config("log_folder",False)}response_{current_ticks}.json", "w") as outfile:
...
server = pywsgi.WSGIServer((get_config("host",False), int(get_config("port",False))), handle_request, spawn=worker_pool)
