...
nuget_prompt = get_config('nuget_prompt', "")

# Get the wrap up cutoff from config
wrap_up_cutoff = get_config('wrap_up_cutoff','')

logging.basicConfig(filename='rig_dotnet8_linux.log', level=logging.INFO)

with open("base_templates/dotnet8_linux/prompt.txt", 'r') as prompt:
...
def run_script(script_path):
    ...
    while True:
        try:
            ...
            script_file = f"{script_path}Program.cs"
            nuget_file = f"{script_path}packages.config"
            ...
            with open(script_file, 'r') as file:
                ...
                try:
                    ...
                    if new_requirements != "":
                        ...
                        with open(nuget_file, 'w') as new_file:
                            new_file.write(new_requirements)
                            
                        git_commit(git_path, "packages.config updated by The Rig")
                        log_message("Commit made for packages.config")
                    else:
                        log_message("No commit made for packages.config - response was empty")
...
# Start the thing
run_script("base_templates/dotnet8_linux/")